...
public class BlurUtils {
     ...
     * @param listener 模糊处理监听器
     */
    public static void blur(Bitmap image, OnBlurProcessListener listener) {
        blur(image, COMPRESS_RATIO, BLUR_RADIUS, 0, listener);
    }
     ...
     * @param listener 模糊处理监听器
     */
    public static void blur(Bitmap image, int delay, OnBlurProcessListener listener) {
        blur(image, COMPRESS_RATIO, BLUR_RADIUS, delay, listener);
    }
     ...
     * @param listener 模糊处理监听器
     */
    public static void blur(final Bitmap bmp, final int compressRatio, final int radius, int delay, final OnBlurProcessListener listener) {
        TaskHelper.execZForSDK(new TaskHelper.Task() {
            private Bitmap blurBmp = null;

            @Override
            public void execute() throws Exception {
                blurBmp = blur(bmp, compressRatio, radius);
            }
    ...
    public static Bitmap blur(Bitmap image) {
        return blur(image, COMPRESS_RATIO, BLUR_RADIUS);
    }

    public static Bitmap blur(Bitmap image, int radius) {
        return blur(image, 1, radius);
    }

    public static Bitmap blur(Bitmap bmp, int compressRatio, int radius) {
        if (bmp == null)
            return bmp;
        ...
        if (!comBmp.equals(bmp))
            comBmp.recycle();
        Logger.v(TAG, "blurImage: usedTime = " + (System.currentTimeMillis() - time));
        return blurBmp;
    }
